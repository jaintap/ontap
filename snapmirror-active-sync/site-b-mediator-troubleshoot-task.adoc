...
. Unmap of the all the luns belonging to the consistency group will fail as it will not be able to replicate the map information from the secondary to primary.

. Modify the igroup and set the replication peer vserver to "-". For example, igroup modify -igroup <igroup name> -replication-peer - -bypass-peer-validation true. This has to be done to all the igroups to which the luns are mapped.

. After modification of the igroup, now delete the lun maps. For example, lun mapping delete -igroup <igroup name> -path <lun path> -vserver <vserver name>
